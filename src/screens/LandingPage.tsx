...
const steps = [
  ...
  ['03', 'Download and sign', "You get SIRA's own PDF, filled in and always on the current version of the form, so it won't come back for being last year's."],
];
const sources = [
...
export function LandingPage({ onEnterApp }: LandingPageProps) {
  return <div className="marketing-page">
    <header className="marketing-header">
      ...
      <nav aria-label="Website navigation"><a href="#how">How it works</a><a href="#cliniko">Cliniko</a><a href="#drafts">Drafts</a><a href="#trust">Your data &amp; compliance</a><a href="#start">Pricing</a></nav>
      <div className="marketing-actions"><button className="marketing-secondary" type="button" onClick={onEnterApp}>Log in</button><a className="marketing-primary" href="#start">Start free trial</a></div>
    </header>
    ...
    <main id="top">
      ...
      <section className="marketing-section" id="how"><p className="marketing-kicker">How it works</p><h2>Note in, form out</h2><p className="section-intro">Paste your notes, check the answers, generate the form. Physios we spoke to were spending about twenty minutes retyping each request. This takes one read-through and a click, and you still sign off on every word.</p><div className="marketing-card-grid">{steps.map(([n,t,b])=><article className="marketing-card" key={n}><strong>{n}</strong><h3>{t}</h3><p>{b}</p></article>)}</div></section>
      ...
      <section className="marketing-section" id="drafts"><h2>Pick up where you left off</h2><p className="section-intro">Requests save as you go, so if you can't finish everything in one sitting, you can pick up exactly where you left off, with your notes, answers and claim details still there.</p><div className="draft-heading"><b>Waiting for you</b><span>4 open</span></div><p className="sample-data-tag">Sample data</p><div className="draft-example-grid">{[['Sample Patient A','SIRA-000001','SIRA NSW','Step 2 of 9'],['Sample Patient B','WS-000002','WorkSafe VIC','Step 4 of 9'],['Sample Patient C','RTW-000003','ReturnToWork SA','Ready to sign'],['Sample Patient D','WC-000004','WorkCover QLD','Step 7 of 9']].map(([n,c,s,p],i)=><div className={i===0?'is-active':''} key={c}><b>{n}</b><code>{c}</code><span>{s}</span><small>{p}</small></div>)}</div></section>

      <section className="marketing-section" id="trust"><p className="marketing-kicker">Your data</p><h2>Patient information stays where it belongs</h2><p className="section-intro">Your notes are the whole input to this product, so here is what happens to them.</p><div className="marketing-card-grid trust-grid">{trust.map(([t,b])=><article className="marketing-card" key={t}><h3>{t}</h3><p>{b}</p></article>)}</div></section>
      ...
      <section className="marketing-start" id="start"><div className="marketing-cta"><div><h2>Try it on your next request</h2><p>Seven days free, no card required. Bring a de-identified note and see the completed form for yourself.</p></div><form onSubmit={e=>e.preventDefault()}><label htmlFor="w-email">Work email</label><input id="w-email" type="email" placeholder="you@yourclinic.com.au"/><label htmlFor="w-state">Where do you practise?</label><select id="w-state" defaultValue="New South Wales"><option>New South Wales</option><option>Victoria</option><option>Queensland</option><option>Commonwealth (Comcare)</option><option>Somewhere else</option></select><button type="submit">Start free trial</button><p className="pricing-note">Then $39 per practitioner per month.</p></form></div></section>
    </main>

    <footer className="marketing-footer"><div><span className="marketing-wordmark">AHTR <span>Assist</span></span><p>Treatment request paperwork for NSW physios, chiros and EPs.</p></div><div><b>Product</b><a href="#how">How it works</a><a href="#cliniko">Cliniko connection</a><a href="#drafts">Saved drafts</a><a href="#start">Pricing</a><a href="#start">Start free trial</a></div><div><b>Contact</b><a href="mailto:hello@ahtrassist.com.au">hello@ahtrassist.com.au</a><a href="tel:+61421736714">0421 736 714</a><span>We reply as quickly as we can.</span></div><div><b>Legal</b><a href="#trust">Your data</a><a href="#compliance">Compliance</a><a href="#privacy">Privacy policy</a><a href="#terms">Terms</a></div><p>© 2026 AHTR Assist</p></footer>
  </div>;
}
